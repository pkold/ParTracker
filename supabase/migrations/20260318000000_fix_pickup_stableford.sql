-- Fix: Pick-up (strokes = 0) should return 0 stableford points, not calculate from net score.
-- Also fix recalculate_round to handle pick-up in net_strokes calculation.

CREATE OR REPLACE FUNCTION calculate_stableford_points(p_net_strokes INT, p_par INT)
RETURNS INT
LANGUAGE plpgsql
IMMUTABLE
AS $$
DECLARE
  v_diff INT;
BEGIN
  -- Pick-up: if net_strokes results from strokes=0 (will be very negative), return 0
  -- We check if net_strokes is unreasonably low (strokes was 0, minus strokes_received)
  IF p_net_strokes <= -10 THEN
    RETURN 0;
  END IF;

  v_diff := p_net_strokes - p_par;

  IF v_diff <= -3 THEN RETURN 5;    -- Albatross or better
  ELSIF v_diff = -2 THEN RETURN 4;  -- Eagle
  ELSIF v_diff = -1 THEN RETURN 3;  -- Birdie
  ELSIF v_diff = 0  THEN RETURN 2;  -- Par
  ELSIF v_diff = 1  THEN RETURN 1;  -- Bogey
  ELSE RETURN 0;                     -- Double bogey or worse
  END IF;
END;
$$;

-- Also update recalculate_round to explicitly handle pick-up (strokes = 0)
...
DECLARE
  ...
  v_lock_key BIGINT;
BEGIN
  -- Advisory lock per round to prevent concurrent recalculation race conditions
  v_lock_key := ('x' || left(replace(p_round_id::text, '-', ''), 15))::bit(64)::bigint;
  ...
  -- Pick-up (strokes = 0): net_strokes = 0, stableford_points = 0
  ...
  SELECT
    ...
    s.strokes,
    CASE WHEN s.strokes = 0 THEN 0
    ELSE calculate_strokes_received(
      rp.playing_hcp,
      (hole_data->>'stroke_index')::int,
      v_round.holes_played
    ) END as strokes_received,
    CASE WHEN s.strokes = 0 THEN 0
    ELSE s.strokes - calculate_strokes_received(
      rp.playing_hcp,
      (hole_data->>'stroke_index')::int,
      v_round.holes_played
    ) END as net_strokes,
    CASE WHEN s.strokes = 0 THEN 0
    ELSE calculate_stableford_points(
  ...
  IF v_round.skins_enabled THEN
    WITH score_for_skins AS (
      SELECT
        ...
        hr.strokes,
        CASE
          WHEN hr.strokes = 0 THEN 999  -- Pick-up loses skins
    ...
    unique_winner AS (
      SELECT
        sfs.hole_no,
        CASE
          WHEN COUNT(*) = 1 AND MIN(sfs.score_for_comparison) < 999 THEN (array_agg(sfs.player_id))[1]
          ELSE NULL
