-- ============================================
-- FIX: Bestball gross/net + Team Skins
-- ============================================
    ...
    IF v_round.team_mode = 'teams' THEN
      ...
      unique_winner AS (
        SELECT
          tbs.hole_no,
          CASE 
            WHEN COUNT(*) = 1 THEN NULL  -- Team winner stored differently
            ELSE NULL
      ...
      -- Note: We can't store team_id in skins_results (it only has player_id)
      -- So we'll store NULL for player and note the winning score
...
SELECT '✅ Fixed: Bestball gross/net + Team skins support' as status;
